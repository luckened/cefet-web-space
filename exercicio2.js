...
const findIndex = () => {
    ...
    imagens.forEach((_, index) => {
        if (servidorDasImagens + imagens[index].arquivo === slide.src) {
            currentIndex = index;
        }
    });
...
previous.addEventListener("click", () => {
    ...
    const index = currentIndex === 0 ? imagens.length - 1 : currentIndex - 1;
...
next.addEventListener("click", () => {
    ...
    const index = currentIndex === imagens.length - 1 ? 0 : currentIndex + 1;
